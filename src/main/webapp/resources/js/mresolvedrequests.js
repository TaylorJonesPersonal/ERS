...
async function loadResolved() {
	console.log("running loadResolved");
	var table = document.getElementById("tableContainer");
	while (table.firstChild) {
		table.removeChild(table.firstChild);
	}
	try {
		let request = await fetch('http://localhost:8080/ERS/api/mgetapproved');
		...
		request = await fetch('http://localhost:8080/ERS/api/mgetdenied');
